...
def _features(records, config=None):
    return FeatureExtractor(config or FeatureExtractionConfig()).extract_records(records).to_dataframe()


def _window_features(record, config):
    return FeatureExtractor().extract_windows(record, config).to_dataframe()
...
def test_extract_features_returns_one_row_per_signal_record() -> None:
    ...
    features = _features(records, config)
    ...
    typed = FeatureExtractor(config).extract_records(records)
    assert typed.feature_columns is not None
    assert "dominant_frequency_hz" in typed.feature_columns
    assert "sampling_rate_hz" not in typed.feature_columns
...
def test_extract_features_rejects_invalid_config() -> None:
    ...
    with pytest.raises(ValueError, match="Invalid frequency band 'bad'"):
        _features(
            [record],
...
def test_sliding_window_features_returns_one_row_per_window() -> None:
    ...
    features = _window_features(record, config)
...
def test_sliding_window_features_rejects_invalid_config() -> None:
    ...
    with pytest.raises(ValueError, match="window_seconds must be positive"):
        _window_features(record, SlidingWindowConfig(0.0, 0.1))

    with pytest.raises(ValueError, match="step_seconds must be positive"):
        _window_features(record, SlidingWindowConfig(0.1, 0.0))
    ...
    with pytest.raises(ValueError, match="step_seconds must not exceed"):
        _window_features(low_rate_record, SlidingWindowConfig(0.04, 0.05))

    with pytest.raises(ValueError, match="must not exceed"):
        _window_features(record, SlidingWindowConfig(2.0, 0.1))
